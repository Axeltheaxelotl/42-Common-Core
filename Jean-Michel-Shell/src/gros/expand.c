...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/07/05 17:35:43 by alanty            #+#    #+#             */
/*   Updated: 2024/07/08 19:44:13 by alanty           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
/*
#include "minishell.h"
...
int	handle_subshell(t_exp_tok *exp_tok)
{
	...
	if (pid == 0)
	{
		cutted_cmd = ft_substr(exp_tok->cmd[0], 1, ft_strrchr(exp_tok->cmd[0], ')') - exp_tok->cmd[0] - 1);
		if (cutted_cmd == NULL)
			return (EXIT_FAILURE);
		status = lexer(cutted_cmd);
		free(cutted_cmd);
...
static int	handle_logical_operators(t_exp_tok *exp_toks[], t_par_tok *par_toks[], int *i)
{
	if ((par_toks[*i]->type == and && get_err_code() != EXIT_SUCCESS)
		|| (par_toks[*i]->type == or && get_err_code() == EXIT_SUCCESS))
		(*i)++;
...
static int	handle_tokens(t_exp_tok *exp_toks[], t_par_tok *par_toks[])
{
	...
	{
		if (par_toks[i]->type == and || par_toks[i]->type == or)
			handle_logical_operators(exp_toks, par_toks, &i);
		else if (par_toks[i]->type == subshell)
			set_err_code(handle_subshell(exp_toks[i]));
		else if (is_redir(par_toks[i]))
			set_err_code(handle_redir(par_toks[i], exp_toks[i], set_pipe_type(par_toks, i)));
		else if (exp_toks[i]->cmd != NULL)
			set_err_code(executor(exp_toks[i]));
		i++;
	}
