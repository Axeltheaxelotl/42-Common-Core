...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/03/21 18:28:42 by alanty            #+#    #+#             */
/*   Updated: 2024/03/21 18:28:44 by alanty           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*liste_vers_str(t_liste **lst)
{
	char	*ligne;
	t_liste	*tmp;
	int		longeur;
...
{
	static t_liste	*lst[1024];

	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	remplir_liste(&lst[fd], dernier(lst[fd]), fd);
	return (liste_vers_str(&lst[fd]));
}
