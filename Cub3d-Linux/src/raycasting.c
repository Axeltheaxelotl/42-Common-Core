...
// charge les textures a partir des fichiers XPM et verifie s'il y a des erreurs
...
int	ft_raycasting(t_recup *recup)
{
	...
	// Rendu des sprites
	ft_sprite(recup);
	// Dessiner la mini-carte avant d'afficher l'image
	draw_minimap(recup);
	// Afficher l'image sur la fenêtre
	// Set image opacity to maximum
	for (int i = 0; i < recup->rx * recup->ry; i++)
	{
